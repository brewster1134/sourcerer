...
module Sourcerer
  class Cli < Thor
    ...
    option :source, aliases: '-s', default: :any, desc: I18n.t('sourcerer.cli.install.options.source.description')
    ...
    def install package_name
      # search for package
      packages = Sourcerer::Package.search package_name, version: options[:version], source: options[:source]
      
      # prompt user to choose a package if multiple are found
      package = if packages.length > 1
        packages_hash = Hash[packages.collect{ |package| [package.location, package] }]
      ...
      # install the package
      S.ay I18n.t('sourcerer.cli.install.installing_package', package: package_name.green, source: package.location.green, destination: options[:destination].green)
      package.install
    end
